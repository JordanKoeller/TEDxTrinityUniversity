...
trait Tables {
   ...
   *  @param speakerId Database column speaker_id SqlType(int4), PrimaryKey
  ...
  class Speakers(_tableTag: Tag) extends profile.api.Table[SpeakersRow](_tableTag, "speakers") {
    ...
    /** Database column speaker_id SqlType(int4), PrimaryKey */
    val speakerId: Rep[Int] = column[Int]("speaker_id", O.PrimaryKey)
